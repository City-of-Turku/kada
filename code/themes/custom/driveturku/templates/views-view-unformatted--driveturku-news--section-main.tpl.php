 ...
 *
 * @todo UGLY HACK: COPIED FROM views-view-unformatted--driveturku-news--front-block.tpl.php
 * BECAUSE THERE IS TOO MUCH LOGIC IN THE TEMPLATE. THIS HAS TO FIXED ASAP!
 */

function _driveturku_news_front_block_space_on_row($content_row) {
  $space_per_row = 4;
  $width = 0;
...
function _driveturku_news_front_block_add_item($content, $width, &$content_row, &$queue, $nid = NULL) {
  $space_on_row = _driveturku_news_front_block_space_on_row($content_row);

  // There's plenty of space for this!
  if ($space_on_row >= $width) {
    $content_row[] = array('id' => $id, 'width' => $width, 'content' => $content, 'nid' => $nid);
  }
  // We can't fit this here right now, let's put it to the queue
  else {
    $queue[] = array('id' => $id, 'width' => $width, 'content' => $content, 'nid' => $nid);
  }
...
function _driveturku_news_front_block_check_queue(&$queue, &$content_row) {
  foreach ($queue as $i => $row) {
    _driveturku_news_front_block_add_item($row['content'], $row['width'], $content_row, $queue, $row['nid']);
...
foreach ($rows as $id => $row) {
  ...
  if ($classes_array[$id]) {
    $content = '<div class="' . $classes_array[$id] .'">' . $item . '</div>';
  }
  else {
    $content = $item;
  }
  ...
  $space_on_row = _driveturku_news_front_block_add_item($row, $width, $content_row, $queue, $node->nid);
  ...
  if ($space_on_row == 0) {
    ...
    _driveturku_news_front_block_check_queue($queue, $content_row);
  }
}
...
while (count($content_rows) < $max_rows && !empty($queue)) {
  ...
  foreach ($queue as $id => $row) {
    ...
    $space_on_row = _driveturku_news_front_block_add_item($row['content'], $row['width'], $content_row, $queue, $row['nid']);
    // The item was added again, either to queue or row, so let's remove the old one
    unset($queue[$id]);
