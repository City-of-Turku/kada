...
function driveturku_links__locale_block(&$variables) {
...
function driveturku_breadcrumb(&$variables) {
...
function driveturku_menu_link(array $variables) {
 ...
 */
function driveturku_process_flexslider_views(&$vars) {
 ...
 */
function driveturku_process_flexslider_list_item(&$vars) {
 ...
 */
function driveturku_flexslider_list(&$vars) {
 ...
 */
function driveturku_service_links_block_format($variables) {
 ...
 */
function driveturku_page_alter(&$page) {
  $path = $GLOBALS['base_url'] . '/' . drupal_get_path('theme', 'driveturku');
  $favicons = array(
    array(
      'rel' => 'icon',
      'type' => 'image/png',
 ...
 */
function _driveturku_term_parents_search($term) {
 ...
 */
function driveturku_ds_pre_render_alter(&$layout_render_array, $context, &$variables) {
 ...
 */
function driveturku_form_element($variables) {
 ...
 */
function driveturku_facetapi_deactivate_widget($variables) {
  return '';
}
 ...
 */
function driveturku_facetapi_count($variables) {
 ...
 */
function driveturku_facetapi_link_active($variables) {
 ...
 */
function driveturku_facetapi_link_inactive($variables) {
  // Sanitizes the link text if necessary.
  $sanitize = empty($variables['options']['html']);
  $link_text = ($sanitize) ? check_plain($variables['text']) : $variables['text'];
  $link_text = '<span class="facet__icon"></span><span class="facet__title">' . $link_text . '</span>';
 ...
 */
function driveturku_select_as_tree($vars) {
 ...
 */
function driveturku_bef_checkbox($variables) {
  $element = $variables['element'];
 ...
 */
function driveturku_select_as_checkboxes($vars) {
  $element = $vars['element'];
...
function driveturku_preprocess_field(&$variables) {
  ...
  if (in_array($variables['element']['#field_name'], $field_names) &&
    ...
    foreach ($variables['items'] as &$item) {
      $item = _driveturku_search_facet_filter_taxonomy_url($item, $field_name, $domain_name);
    }
  }
...
function _driveturku_search_facet_filter_taxonomy_url(&$item, $field_name, $domain_name) {
 ...
 */
function driveturku_date_display_range($variables) {
 ...
 */
function driveturku_feed_icon($variables) {
  $text = t('Subscribe to feed');
  if ($image = theme('image', array('path' => drupal_get_path('theme', 'driveturku') . '/images/rss-icon.svg', 'width' => 16, 'height' => 16, 'alt' => $text))) {
 ...
 */
function driveturku_js_alter(&$js) {
  // Load behaviours last
  $js['sites/all/themes/custom/driveturku/js/driveturku.behaviors.js']['scope'] = 'footer';
  $js['sites/all/themes/custom/driveturku/js/driveturku.behaviors.js']['weight'] = 100;
}
